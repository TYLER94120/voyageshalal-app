...
export const REGLES_DOUTEUX = [
    ...
    },
    {
        ...
        raison: "Pepsine, pancréatine et lipase sont souvent extraites d'estomac ou de pancréas ; il en existe aussi des versions microbiennes. L'étiquette ne le précise pas.",
...
function normaliser(texte) {
    ...
    t = t.replace(/(?:mono-? et diglycerides|mono-?diglycerides|stearates? de magnesium|stearates? de calcium|acides? stearique|acides? gras)(?:(?!anim)[^,;.])*?(?:d'origine\s+)?vegetale?s?/g, " corps gras vegetal ");
 ...
 */
const MENTIONS_ABSENCE = /non renseign\w*|non sp[ée]cifi\w*|non communiqu\w*|informations? non disponibles?|non disponibles?|pas d'informations?|aucune information|liste non disponible|ingr[ée]dients? non disponibles?|[àa] compl[ée]ter|[àa] renseigner|voir (?:sur )?(?:l'|le )?emballage|see (?:the )?packaging|not available|no information|unknown/gi;
/**
 ...
 */
const DEBUT_DES_TRACES = /(?:peut contenir|peuvent contenir|traces? eventuelles?|traces? possibles?|traces? de|fabrique dans un (?:atelier|etablissement)|susceptible de contenir|presence possible)/;
function separerLesTraces(texte) {
    const m = texte.match(DEBUT_DES_TRACES);
    if (!m || m.index === undefined)
        return { composition: texte, traces: "" };
...
export function analyserProduit(entree) {
    ...
    const codes = [...((_c = entree.additifs) !== null && _c !== void 0 ? _c : []), ...codesEDuTexte(texte)];
    ...
    if (texte.trim().length > 0) {
        ...
        for (const regle of [...REGLES_HARAM.map((r) => ({ ...r, niveau: "haram" })), ...REGLES_DOUTEUX.map((r) => ({ ...r, niveau: "douteux" }))]) {
            if (regle.motif.test(composition)) {
                ...
                alertes.push({
                    ...
                    niveau: "douteux",
                    raison: "L'étiquette signale des traces possibles, pas un ingrédient : le produit n'en contient pas. " +
    ...
    for (const a of alertes) {
        const cle = ((_d = a.famille) !== null && _d !== void 0 ? _d : a.element).toLowerCase();
